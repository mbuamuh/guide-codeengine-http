...
:projectid: cloud-ibm-codeengine-http
...
:inventory-get: curl -X GET https://inventoryapp.[xxxxxx].[cloud region].codeengine.appdomain.cloud/inventory/systems
:inventory-post: curl -X POST https://inventoryapp.[xxxxxx].[cloud region].codeengine.appdomain.cloud/inventory/systems/systemapp.[xxxxxx].[cloud region].codeengine.appdomain.cloud
...
You will learn how to deploy two microservices in containers, to IBM Cloud Container Registry and then to IBM Cloud Code Engine, to run as serverless functions in IBM Cloud. You will also learn how to send and receive data from the developed serverless functions, using Http as the event source.
...
* *IBM Cloud Container Registry plugin CLI:* 
You need the IBM Cloud Container Registry command-line tool plugin `ibmcloud cr` to set up your own image namespace in an IBM-hosted, and managed, private registry. You will store Docker images of the `system` and `inventory` microservices in a namespace in the private registry. The microservices will be deployed into IBM Cloud Code Engine, from the private registry. To install the IBM Cloud Container Registry plugin CLI on your platform, run the following command:
+
[role=command]
```
...
* *IBM Cloud Code Engine plugin CLI:*
You need the IBM Cloud Code Engine command-line tool plugin `ibmcloud ce` to access the Code Engine service in your IBM Cloud account. To install the IBM Cloud Code Engine plugin CLI on your platform, run the following command: 
+
[role=command]
```
...
----
ibmcloud iam api-key-create guideKey -d "Key to demo the open liberty serverless microservices, using code engine" --file guideKey.txt
----
...
Since you built a new image, you need to push it to the repository again. In the following `docker` command, remember to replace `uk.icr.io` with your registry and `guide-namespace` with the namespace you created earlier in the guide.
...
[role=command]
```
docker tag system:2.0-SNAPSHOT uk.icr.io/guide-namespace/system:2.0-SNAPSHOT
```
...
[role=command]
```
docker push uk.icr.io/guide-namespace/system:2.0-SNAPSHOT
```
...
[role=command]
```
ibmcloud ce app update --name systemapp --image uk.icr.io/guide-namespace/system:2.0-SNAPSHOT --registry-secret guide-registry --port 9080
```
...
Remove your images from your container registry. Remember to replace `uk.icr.io` to your registry and `guide-namespace` with the namespace you created earlier in the guide.

[role=command]
```
ibmcloud cr image-rm uk.icr.io/guide-namespace/system:1.0-SNAPSHOT
ibmcloud cr image-rm uk.icr.io/guide-namespace/system:2.0-SNAPSHOT
ibmcloud cr image-rm uk.icr.io/guide-namespace/inventory:1.0-SNAPSHOT
```
...
Log out of your container registry. Remember to replace `uk.icr.io` to your registry.

[role=command]
```
docker logout uk.icr.io
