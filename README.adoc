...
:system-properties: {system-api}/system/properties
...
[role='command']
```
curl -X GET https://systemapp.[xxxxxx].[cloud region].codeengine.appdomain.cloud
```
...
[role='command']
```
curl -X GET https://inventoryapp.[xxxxxx].[cloud region].codeengine.appdomain.cloud/inventory/systems
```
...
[role='command']
```
curl -X POST https://inventoryapp.[xxxxxx].[cloud region].codeengine.appdomain.cloud/inventory/systems/systemapp.[xxxxxx].[cloud region].codeengine.appdomain.cloud -H "Ce-Specversion: 1.0" -H "Ce-Type: java.properties" -H "Ce-Source: io.cloudevents.examples/properties" -H "Ce-Id: properties" -H "Content-Type: application/json" -H "Ce-Subject: resources" -d "[\"java.vendor.url\",\"wlp.install.dir\"]"
```
...
When you no longer need the serverless applications, you can delete them from IBM Cloud code engine, and all the resources you created in order to deploy the applications.
